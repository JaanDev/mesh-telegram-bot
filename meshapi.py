...
def get_user(tg_id) -> database.Users | None:
    with database.engine.begin() as conn:
        req = conn.execute(select(database.Users).where(database.Users.tg_id == tg_id))
        data = req.one_or_none()
        return data
...
async def schedule(chat_id, date1: datetime, date2: datetime):
    ...
    date = date1
    while True:
        urls.append(f"https://school.mos.ru/api/family/mobile/v1/schedule?student_id={user.student_id}&date={date.strftime('%Y-%m-%d')}")
...
async def homework(chat_id, date1: datetime, date2: datetime):
    ...
    data = requests.get(f'https://dnevnik.mos.ru/core/api/student_homeworks?begin_prepared_date={date1.strftime("%d.%m.%Y")}&end_prepared_date={date2.strftime("%d.%m.%Y")}&student_profile_id={user.student_id}', headers={
        "Auth-token": user.token,
        "Profile-Id": user.student_id
    }, cookies={
...
async def marksdate(chat_id, date1: datetime, date2: datetime):
    ...
    data = requests.get(f"https://dnevnik.mos.ru/core/api/marks?created_at_from={date1.strftime('%d.%m.%Y')}&created_at_to={date2.strftime('%d.%m.%Y')}&student_profile_id={user.student_id}", headers={
        "Auth-token": user.token,
        "Profile-Id": user.student_id
    }, cookies={
        'auth_token': user.token,
...
async def marks(chat_id):
    ...
    this_year = [year['id'] for year in years.json() if year['current_year']][0]

    data = requests.get(f'https://dnevnik.mos.ru/reports/api/progress/json?academic_year_id={this_year}&student_profile_id={user.student_id}', headers={
        'Auth-Token': user.token,
        'Profile-Id': user.student_id
    }, cookies={
        'auth_token': user.token,
...
async def notifications(chat_id):
    ...
    data = requests.get(f"https://school.mos.ru/api/family/mobile/v1/notifications/search?student_id={user.student_id}", headers={
        'Auth-Token': user.token,
        'Profile-Id': user.student_id,
        "x-mes-subsystem": "familymp"
    })
...
async def try_add_new_token(token, tg_id) -> bool:
    global db
    ...
    with database.sessions.begin() as session:
        session.merge(database.Users(tg_id=tg_id, token=token, student_id=student_id))
